...
<div class="portlet box green">
    ...
    <div class="portlet-body form">
        <form id="userForm" class="form-horizontal form-bordered" action="/user/save" method="post"
            ...
            <div class="form-group">
                ...
                <div class="col-md-4">
                    <div class="input-icon right">
                        ...
                        <input type="text" class="form-control" id="orgName" name="orgName" onclick="showOrg()"
                               readonly value="${user.orgName}">
                    </div>
                </div>
                <div class="col-md-2">
                        ...
                        <select class="form-control select2" style="padding: 0;" id="roleId" multiple>
                            <c:forEach items="${roleList}" var="role">
                                <option value="${role.id}"
                                        <c:if test="${role.userHas}">selected</c:if>>${role.roleName}</option>
                            </c:forEach>
                        </select>
            ...
            <div class="form-group">
                ...
                <div class="col-md-4">
                    <div class="input-icon right">
                        <i class="fa"></i>
                        <select name="rotationId" class="form-control" required>
                            <option value="">请选择</option>
                            <c:forEach items="${isPositive}" var="isPositive">
                                <option
                                        <c:if test="${isPositive.code == user.rotationId}">selected</c:if>
            ...
            <div class="form-group">
                <label class="col-md-3 control-label">是否在职:</label>
                <div class="col-md-4">
                    <div class="input-icon right">
                        <i class="fa"></i>
                        <select name="isWork" class="form-control" required>
                            <option value="">请选择</option>
                            <c:forEach items="${isWork}" var="isWork">
                                <option
                                        <c:if test="${isWork.code == user.isWork}">selected</c:if>
                                        value="${isWork.code}">${isWork.name}</option>
                            </c:forEach>
                        </select>
                    </div>
                </div>
            ...
            <div class="form-group">
                <label class="col-md-3 control-label">直属上级:</label>
                <div class="col-md-4">
                    <div class="input-icon right">
                        <i class="fa"></i>
                        <input type="hidden" id="parentId" name="parentId" value="${user.parentId}"/>
                        <input type="text" class="form-control" id="parentName" name="parentName" onclick="showOrgs()"
                               readonly
                               value="${user.parentName}" maxlength="200"/>
                    </div>
                </div>
                <div class="col-md-2">
                ...
                <div class="col-md-4">
                    <div class="input-icon right">
                        ...
                        <textarea rows="5" class="form-control" name="description"
                                  maxlength="255">${user.description}</textarea>
                    </div>
                </div>
            </div>
...
<script type="text/javascript">
    ...
    function showOrgs() {
        ...
        $.ajax({
            type: 'post',
            url: '<c:url value="/user/getOrgForTrees"></c:url>',
